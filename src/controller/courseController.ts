...
export const createCourse = catchAsyncErrors(
  async (req: IGetUserAuthInfoRequest, res: Response, next: NextFunction) => {
    ...
    const myCloud = await cloudinary.v2.uploader.upload(fileUrl.content as string);
...
export const getCourseLectures = catchAsyncErrors(async (req:Request, res:Response, next:NextFunction) => {
  const course = await Course.findById(req.params.id) as ICourseSchemaDocument;
...
export const addLectures = catchAsyncErrors(async (req:IGetUserAuthInfoRequest, res:Response, next:NextFunction) => {
...
export const deleteLecture = catchAsyncErrors(async (req:IGetUserAuthInfoRequest, res:Response, next:NextFunction) => {
    const { lectureId, courseId }: { lectureId: string; courseId: string } = req.query as {
        lectureId: string;
        courseId: string;
      };
